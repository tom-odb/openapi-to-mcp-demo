...
Composite tools use an LLM agent to orchestrate multiple API calls.
"""
import asyncio
import httpx
import json
...
class GenericMCPServer:
    ...
    async def _handle_composite_tool(self, tool: Dict[str, Any], arguments: dict) -> List[TextContent]:
        """Handle a composite multi-endpoint tool using LLM orchestration"""
        
        if not self.anthropic_api_key:
            return [TextContent(
                type="text", 
                text="Error: Composite tools require ANTHROPIC_API_KEY to be set. "
                     "This tool orchestrates multiple API calls using an LLM agent."
            )]
        ...
        # Create tool definitions for the LLM agent
        agent_tools = []
        ...
        system_prompt = f"""You are an API orchestration agent. Your job is to execute a complex workflow by calling multiple API endpoints in the correct order and combining their results.
...
AVAILABLE TOOLS: You have access to the following API endpoint tools:
{json.dumps(agent_tools, indent=2)}

INSTRUCTIONS:
1. Call the API tools in the correct order based on the orchestration logic
...
Execute the workflow step by step, calling tools as needed."""
        
        # Initialize Anthropic client
        client = Anthropic(api_key=self.anthropic_api_key)
        messages = []
        
        print("[MCP] Starting LLM orchestration agent...")
        ...
        try:
            ...
            while iteration < max_iterations:
                ...
                response = client.messages.create(
                    ...
                    messages=messages if messages else [{"role": "user", "content": "Execute the workflow."}],
                    tools=agent_tools
                )
                ...
                if response.stop_reason == "tool_use":
                    ...
                    for block in response.content:
                        if block.type == "tool_use":
                            ...
                            print(f"[MCP]   → Calling tool: {tool_name}({json.dumps(tool_input)})")
                            
                            # Find the standard tool and execute it
                            tool_found = False
                            for standard_tool in self.tools:
